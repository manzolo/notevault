...
4. **Pubblica su Docker Hub**
   - `make publish APP_VERSION=X.Y.Z`

5. **Crea GitHub release**
   - `gh release create vX.Y.Z --generate-notes --title "vX.Y.Z"`

6. **Deploy su server di produzione**
   - `make deploy-update APP_VERSION=X.Y.Z`

7. **Push branch main**
   - `git push origin main`

8. **Pulizia immagini Docker vecchie** (mantieni nuova, precedente, latest)
   - Locale:
     ```
...
9. **Aggiorna la memoria** — aggiorna `MEMORY.md` con la nuova versione corrente
