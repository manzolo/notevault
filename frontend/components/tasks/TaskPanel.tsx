...
function TaskRow({ task, onToggle, onDelete, onUpdate, onArchive }: {
  ...
  return (
    ...
    <div
      ...
      className="group py-2 border-b border-gray-100 dark:border-gray-700/50 last:border-0"
    >
      {/* Main row: drag + checkbox + title */}
      <div className="flex items-start gap-2">
        {!task.is_done && (
          <span
            {...attributes}
            {...listeners}
            className="cursor-grab active:cursor-grabbing text-gray-300 hover:text-gray-500 dark:text-gray-600 dark:hover:text-gray-400 shrink-0 select-none px-0.5 mt-0.5"
      ...
      <div className={`flex items-center gap-2 mt-1 ${!task.is_done ? 'pl-10' : 'pl-6'}`}>
          ...
          <DateTimePicker
            ...
            triggerClassName={
              task.due_date
                ...
                : 'text-gray-300 hover:text-indigo-400 dark:text-gray-600 dark:hover:text-indigo-400 transition-colors disabled:cursor-default text-xs'
            }
          />
        </div>
        {onArchive && (
          <button
            ...
            title={tc('archive')}
            className="sm:opacity-0 sm:group-hover:opacity-100 text-gray-400 hover:text-amber-500 transition-opacity"
          >
        ...
        {task.id && !task.is_done && (
          <button
            ...
            title={t('reminders')}
            className={`cursor-pointer text-gray-400 hover:text-indigo-500 dark:hover:text-indigo-400 transition-opacity ${
              showReminders ? 'opacity-100 text-indigo-500 dark:text-indigo-400' : 'sm:opacity-0 sm:group-hover:opacity-100'
            }`}
          >
